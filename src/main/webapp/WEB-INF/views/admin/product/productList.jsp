									...
									<tbody>
										...
										<c:forEach items="${productList }" var="productVO" varStatus="status">
											<tr role="row"
												...
												<td>
													<input type="checkbox" class="check" value='<c:out value="${productVO.pdt_num }"></c:out>'>
													<input type="hidden" name="pdt_img" value='<c:out value="${productVO.pdt_img }"></c:out>'>
													<input type="hidden" name="pdt_uploadpath" value='<c:out value="${productVO.pdt_uploadpath }"></c:out>'>
												</td>
												<td class="sorting_1"><c:out value="${productVO.pdt_num }"></c:out></td>
												<td>
												<a class="move"
														href="<c:out value="${productVO.pdt_num }"></c:out>">
													<img name="productImage" src="/admin/product/displayFile?fileName=s_<c:out value="${productVO.pdt_img }"></c:out>&uploadPath=<c:out value="${productVO.pdt_uploadpath }"></c:out>">
												</a>
													<input type="text" value='<c:out value="${productVO.pdt_name }"></c:out>'>
												</td>
												
												<td><fmt:formatDate value="${productVO.pdt_date }" pattern="yyyy-MM-dd" /></td>
												<td><input type="text" value='<c:out value="${productVO.pdt_price }"></c:out>'></td>
												<td>
													<input type="text" value='<c:out value="${productVO.pdt_amount }"></c:out>'>
												</td>
												<td>
													<input type="checkbox" value="<c:out value="${productVO.pdt_buy }"></c:out>" 
														<c:out value="${productVO.pdt_buy == 'Y' ? 'checked': '' }"></c:out>>
												</td>
												<td><input type="button" name="btnProductModify" value="수정" data-pdt_num='<c:out value="${productVO.pdt_num }"></c:out>'></td>
												<td><input type="button" name="btnProductDelete" value="삭제" data-pdt_num='<c:out value="${productVO.pdt_num }"></c:out>'></td>
											</tr>
										</c:forEach>
									</tbody>
...
<script>
	...
	$(document).ready(function(){
		...
		$("#btnCheckDelete").on("click", function(){
			...
			//자바스크립트 배열
			let pdt_numArr = []; //상품코드 배열
			let pdt_imgArr = []; //상품이미지 배열
			let pdt_uploadpathArr = []; //날짜폴더이름 

			//선택된 체크박스 일 경우
			$(".check:checked").each(function(){
				let pdt_num = $(this).val();
				let pdt_img = $(this).next().val();
				let pdt_uploadpath = $(this).next().next().val();

				pdt_numArr.push(pdt_num);
				pdt_imgArr.push(pdt_img);
				pdt_uploadpathArr.push(pdt_uploadpath);
			})
			...
			*/
			...
			$.ajax({
				...
				dataType: 'text',
				data:  {
					pdt_numArr : pdt_numArr,
					pdt_imgArr : pdt_imgArr,
					pdt_uploadpathArr : pdt_uploadpathArr
				},
